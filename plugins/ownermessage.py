...
async def owner_msg_command(client: Client, message: Message):
    ...
    await message.reply_text(
        f"👋 Hey {user_mention},\n\nPlease send your message now, and it will be forwarded to the owner.",
        parse_mode=ParseMode.HTML
    )

# Capture and forward the user's message
@Bot.on_message(filters.private & filters.text & ~filters.command)
async def forward_message_to_owner(client: Client, message: Message):
    user_mention = message.from_user.mention

    # Forward the user's message to the owner
    await client.send_message(
        chat_id=OWNER_ID,
        text=f"<b>📩 Message from {user_mention}:</b>\n\n{message.text}",
        parse_mode=ParseMode.HTML
    )
